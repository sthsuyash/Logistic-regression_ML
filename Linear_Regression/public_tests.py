...
def compute_cost_test(target):
    ...
    cost = target(x, y, initial_w, initial_b)
    assert np.isclose(
        cost, 15.325), f"Case 3: Cost must be 15.325 for a perfect prediction but got {cost}"

    # Case 4
    initial_b = 1.0
    cost = target(x, y, initial_w, initial_b)
    assert np.isclose(
        cost, 10.725), f"Case 4: Cost must be 10.725 but got {cost}"
    ...
    cost = target(x, y, initial_w, initial_b)
    assert np.isclose(
        cost, 4.525), f"Case 5: Cost must be 4.525 but got {cost}"
...
def compute_gradient_test(target):
    ...
    assert np.allclose(
        dj_dw, -10.0), f"Case 1: dj_dw is wrong: {dj_dw} != -10.0"
